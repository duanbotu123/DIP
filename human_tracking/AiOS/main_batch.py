...
def main(args):
    ...
    root_paths = [os.path.join(args.inference_input, path) for path in natsorted(os.listdir(args.inference_input+'_dwpose'))]
    root_paths = root_paths[:4]
    for root_path in tqdm(root_paths):
        # id_name = root_paths.split("/")[-1]
        root_vid = root_path+'/vid/0.mp4'
        out_dir = root_path+'/aios'
        os.makedirs(out_dir, exist_ok=True)

        if not args.inference:
            dataset_val = eval(cfg.testset)(transforms.ToTensor(), "test")
        else:
            dataset_val = eval(cfg.testset)(root_vid,out_dir)
        ...
        if args.eval:
            ...
            if root_vid is not None and args.inference:
                inference(model,
                        criterion,
                        postprocessors,
                        data_loader_val,
