...
def cal_dis(p1, p2):
    return torch.sqrt(torch.sum(torch.pow(torch.abs(p1 - p2) + 1e-5, 2))).item()

# file_dir_path = os.path.dirname(os.path.realpath(__file__))
file_dir_path = 'portrait_magic/dmm_models/adnerf_rendering'
part = 'wholebody'
verts, faces, aux = load_obj(os.path.join(file_dir_path, part + '_no_teeth_uv.obj'))
print(verts.shape)
...
teeth_info = np.loadtxt(os.path.join(file_dir_path, 'teeth_tris.txt'), dtype=np.float32)
