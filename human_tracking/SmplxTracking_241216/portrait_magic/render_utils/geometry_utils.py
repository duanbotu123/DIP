...
def compute_vertex_normals(vertices, faces):
    ...
    if (faces.shape[0] == 1) and (bs > 1):
        faces = faces.expand(bs, -1, -1)
...
def proj_pts_mv(rott_geo, cam_para):
    '''
    rott_geo: (v, b, n, 3)
    cam_para: (v, b, 4) -> (v, b, n, 2)
    '''
    # 从 cam_para 中提取内参，形状均为 (v, b, 1)
    fx = cam_para[..., 0:1]
    fy = cam_para[..., 1:2]
    cx = cam_para[..., 2:3]
    cy = cam_para[..., 3:4]

    # 提取 rott_geo 中的 X, Y, Z 坐标，形状为 (v, b, n)
    X = rott_geo[..., 0]
    Y = rott_geo[..., 1]
    Z = rott_geo[..., 2]
    ...
    fxX = fx * X      # (v, b, n)
    fyY = fy * Y      # (v, b, n)
    proj_x = fxX / Z + cx  # (v, b, n)
    proj_y = fyY / Z + cy   # (v, b, n)
