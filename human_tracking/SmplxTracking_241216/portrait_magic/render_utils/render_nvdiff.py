...
class MeshRenderer_Cuda(nn.Module):
    ...
    def forward_visualization_geo(self, vertices_cam, tris, cam_para, img_size, img_ori = None, return_rast_out = False):
        ...
        with torch.inference_mode():
            ...
            if img_ori is not None:
                ...
                vis_img = (1 - mask) * img_ori + mask * (vis_img*1. + img_ori*.0)
            else:
                vis_img = vis_img[:, :img_size[0], :img_size[1]]
        if not return_rast_out:
            return torch.clamp((vis_img*255.).detach().byte(), 0, 255).cpu().numpy()
    ...
    def forward_differentiable_mask_normal(self, vertices_cam, tris, cam_para, img_size):
        '''
        vertices_cam: (b, nv, 3), tris: (b, nf, 3), cam_para: (b, 4), img_size: [h, w]
        '''
        if self.glctx is None:
        ...
        vertices_normals = compute_vertex_normals(vertices_cam, tris)
        vertices_mask = torch.ones_like(vertices_cam[..., :1])
        rendered_all = dr.interpolate(torch.cat((vertices_mask, vertices_normals), dim=-1), rast_out, tris[0], rast_db=rast_db, diff_attrs='all')[0]
        rendered_all = dr.antialias(rendered_all, rast_out, vertices_ndc.contiguous(), tris[0])
        rendered_normal = nn.functional.normalize(rendered_all[:, :img_size[0], :img_size[1], 1:], dim=-1)
        return rendered_all[:, :img_size[0], :img_size[1], :1], rendered_normal
