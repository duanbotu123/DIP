...
class LDMK_Fitting():
    def __init__(self):
        ...
        self.temp_body_pose = np.load(os.path.join(smplx_model_path, 'template_body_pose.npy'))

        self.mesh_renderer = MeshRenderer_Cuda().cuda()
        self.confidence_min = .75
        ...
        self.shared_keys = ['shape'] ### shared across frames
        self.no_opt_keys = ['lower_body_pose', 'lhand_pose', 'rhand_pose', 'shape']
        ...
        self.confidence_scale[:, np.arange(0,5), :] = .1
        self.confidence_scale[:, [5,6], :] = .0
        self.confidence_scale[:, torch.arange(7, 23), :] = 0.
        self.confidence_scale[:, 91:, :] = 0. 
        
        self.smoother_buffers, self.smoother_wts, self.smoother_pre_values = {}, {}, {}
        self.smoother_wts['verts'] = 1e3 * 0.
        self.smoother_wts['upper_body_poses'] = 1e3 * 1.
        self.smoother_wts['lower_body_poses'] = 1e4 * 0.
        self.smoother_wts['global_orient'] = 1e3 * 1.
        self.smoother_wts['transl'] = 1e2 * 1.
        self.smoother_wts['expr'] = 1. * 0.
        self.smoother_wts['jaw'] = 2. * 0.
        self.upper_body_constant_weight = torch.ones((1, 7, 1), dtype=torch.float32).to(self.device) * 1.
        self.upper_body_constant_weight[0, [3, 6], 0] = .1
    ...
    def cal_loss(self, print_info = None):
        paras_dict = {}
        confidence = None
        ...
        if self.with_losses_dict['smooth']:
            ...
            for smoother_key in self.smoother_wts.keys():
                ...
                elif smoother_key == 'upper_body_poses':
                    loss_smooth += cal_smooth_loss(concat_values[smoother_key], attn_weight=self.upper_body_constant_weight) * self.smoother_wts[smoother_key]
                else:
        ...
        if self.with_losses_dict['mask']:
            ...
            if self.with_losses_dict['normal']:
                ...
                normal_valid_mask[(self.parsing_maps == 14) | (self.parsing_maps == 16) | (self.parsing_maps == 15) | (self.parsing_maps == 11) | (self.parsing_maps == 0) | (self.parsing_maps == 3) | (self.parsing_maps == 1) | (self.parsing_maps == 18)] = 0.
                # print(normal_valid_mask.shape)
        ...
        ldmk_mask_ratio = loss_ldmks.item() / (loss_mask.item() + 1e-5) * self.ldmk_mask_ratio * 0.
        ldmk_normal_ratio = loss_ldmks.item() / (loss_mask.item() + 1e-5) * .1 * .0
        ldmk_feature_ratio = loss_ldmks.item() / (loss_feature.item() + 1e-5) * self.ldmk_feature_ratio

        return (loss_ldmks*self.ldmk_ratio + loss_mask*ldmk_mask_ratio  + loss_feature*ldmk_feature_ratio + loss_normal*ldmk_normal_ratio) * 1. + loss_shape * 4e-3 * 0.1 + loss_exp *.25 + loss_smooth + loss_iris * .3
        

    def opt_smplx_paras(self, paras_dict_init, cam_para, pts2d_gt, iris_ldmks, is_opt_dict, with_losses_dict, confidence = None, with_fixed_bary = False, adam_lr = 1e-2, iter_nums = 300, ldmk_ratio = 1., ldmk_mask_ratio = 1., ldmk_feature_ratio = .2, current_step = None):
    ...
    def load_masks(self, mask_folder, ldmks_names, dst_size):
        ...
        for ldmk_name in ldmks_names:
            ...
            normal_maps.append(normal_map.astype(np.float32)/255.*2.-1.)
    ...
    def run_folder(self, imgs_folder, save_folder, debug_dir='none'):
        ...
        recon_params = torch.load(os.path.join(imgs_folder.replace('ori_imgs', 'smplx_recon'), 'smplx_track.pth'))
        self.frames_params['shape'] = torch.as_tensor(recon_params['shape'][:1])
        ...
        self.frames_params['upper_body_pose'] = torch.as_tensor(self.temp_body_pose).reshape(1, 21, 3).repeat(frames_num, 1, 1)[:, self.upper_body_inds, :]
        self.frames_params['lower_body_pose'] = torch.as_tensor(self.temp_body_pose).reshape(1, 21, 3).repeat(frames_num, 1, 1)[:, self.lower_body_inds, :]
        for para_key in self.paras_keys:
            if para_key in self.shared_keys:
        ...
        for i in tqdm(range(0, frames_num, batch_size), desc = 'opt global_orient and transl'):
            ...
            is_opt_dict.update({'globel_orint': True, 'transl': True})
            ...
            for j in range(0, end_id-start_id):
                ...
                render_vis = self.mesh_renderer.forward_visualization_geo(smplx_out.vertices[j:j+1], self.tris, cam_para, img.shape[:2], torch.as_tensor(img).to(self.device)[None, :, :, [2,1,0]].float()/255.)
        ...
        #     if key not in self.no_opt_keys:
        #         is_opt_dict[key] = True
        # with_losses_dict = deepcopy(self.with_losses_dict_init)
        # with_losses_dict.update({'mask': True, 'normal': True, 'feature': True})
        # dict_for_subinds = self.opt_smplx_paras(dict_for_subinds, cam_para.expand(cur_batch_size, -1), ldmks_2d[cur_ids], iris_ldmks_all[cur_ids], is_opt_dict, with_losses_dict, confidence = ldmks_confidence[cur_ids], adam_lr = 3e-3, iter_nums=600, ldmk_ratio=1.5, ldmk_feature_ratio=0., current_step='opt shared shape')
        # for key in self.paras_keys:
        #     if key in self.shared_keys:
        #         self.frames_params[key] = dict_for_subinds[key].clone()
        #     # else:
