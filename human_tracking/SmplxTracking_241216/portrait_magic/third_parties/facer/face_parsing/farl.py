...
from kornia.geometry.transform import warp_affine, invert_affine_transform
...
class FaRLFaceParser(FaceParser):
    ...
    def forward_with_imgs_points(self, images, points):
        tgt_size = 448
        setting = pretrain_settings[self.conf_name]
        _, _, h, w = images.shape
        matrix = setting['get_matrix_fn'](points)
        # print(matrix)
        inv_matrix = invert_affine_transform(matrix[:, :2])
        
        w_images = warp_affine(images, matrix[:, :2], (tgt_size, tgt_size), align_corners=False)
        w_seg_logits, _ = self.net(w_images)  # (b*n) x c x h x w
        w_seg_probs = w_seg_logits.softmax(dim=1) 
        w_seg_labels = w_seg_probs.argmax(dim=1, keepdim=True).float()
        # print(w_images.shape, w_seg_logits.shape, w_seg_probs.shape, w_seg_labels.shape)
        inv_matrix[:, :2, :2] *= float(tgt_size) / w_seg_logits.shape[-1]
        seg_labels = warp_affine(w_seg_labels, inv_matrix[:, :2], (h, w), mode='nearest', align_corners=False)
    ...
    #         images, grid, mode='bilinear', align_corners=False)
    ...
    #     seg_labels = seg_probs.argmax(dim=1, keepdim=True).float()
