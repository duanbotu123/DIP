...
class LDMK_Fitting():
    ...
    def run_folder(self, imgs_folder, save_folder, debug_dir='none'):
        ...
        self.frames_params['upper_body_pose'] = torch.as_tensor(recon_params['body_pose']).reshape(-1, 21, 3)[:, self.upper_body_inds, :]
        self.frames_params['lower_body_pose'] = torch.as_tensor(recon_params['body_pose']).reshape(-1, 21, 3)[:, self.lower_body_inds, :]
        self.frames_params['lhand_pose'] = torch.as_tensor(recon_params['lhand_pose'])
        self.frames_params['rhand_pose'] = torch.as_tensor(recon_params['rhand_pose'])
        cam_para = torch.tensor(recon_params['cam_para']).cuda().reshape(1, 4)
