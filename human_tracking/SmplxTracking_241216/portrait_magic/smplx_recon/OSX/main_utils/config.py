...
class Config:
    ...
    feat_dim = 1024
    num_noise_sample = 0
    encoder_config_file = os.path.join(root_dir, 'main_utils/transformer_utils/configs/osx/encoder/body_encoder_large.py')
    ...
    def set_additional_args(self, **kwargs):
        ...
        if self.encoder_setting == 'osx_b':
            self.encoder_config_file = os.path.join(cfg.root_dir, 'main_utils/transformer_utils/configs/osx/encoder/body_encoder_base.py')
            self.encoder_pretrained_model_path = '../pretrained_models/osx_vit_b.pth'
            self.feat_dim = 768
        elif self.encoder_setting == 'osx_l':
            self.encoder_config_file = os.path.join(cfg.root_dir, 'main_utils/transformer_utils/configs/osx/encoder/body_encoder_large.py')
