...
class GaussianScene(nn.Module):
    def __init__(self, data_dir, img_size, head_smplx_path, sh_degree = 3, device = 'cuda:0'):
        super().__init__()
        self.data_dir = data_dir
        self.img_size = img_size
        self.head_smplx_path = head_smplx_path
    ...
    def laplacian_smooth(self, params):
        padded_params = torch.cat((params[:1], params, params[-1:]), dim=0)
        smo_rate = .5
        padded_params[1:-1] = padded_params[1:-1]*(1-smo_rate) + padded_params[:-2]*smo_rate*.5 + padded_params[2:]*smo_rate*.5
        return padded_params[1:-1]
    ...
    def restore(self, model_dict):
        ...
        self.body_pose += _body_pose_dis
        body_pose = self.body_pose.clone().reshape(-1, 21, 3)
        self.body_pose = body_pose.reshape(-1, 63)
    ...
    def update_xyz_rot(self, frame_ids, with_mesh_rendering = True, in_train = False, mode = 'body'):
        ...
        if not in_train: ### only return mesh vis
            mesh_vis = self.mesh_renderer.forward_visualization_geo(verts_cam, self.geo_creator.tris_for_mesh_render.expand(batch_size, -1, -1), self.cam_para.expand(batch_size, -1), self.img_size)
            return mesh_vis
    ...
    def load_smplx_track(self):
        ...
        smplx_track_dict_ano = torch.load(self.head_smplx_path)
        real_num = smplx_track_dict_ano['jaw_pose'].shape[0]
        self.jaw_pose[-real_num:] = torch.from_numpy(smplx_track_dict_ano['jaw_pose']).to(self.device)
        self.expr[-real_num:] = torch.as_tensor(self.expr[:real_num]).to(self.device)

        self.leye_pose[-real_num:] = torch.as_tensor(self.leye_pose[:real_num]).to(self.device)
        self.reye_pose[-real_num:] = torch.as_tensor(self.reye_pose[:real_num]).to(self.device)
        
        self.expr[-real_num:] = self.expr[-real_num:]*.0 + torch.as_tensor(smplx_track_dict_ano['expr']).to(self.device)*.7
        ...
        for i in range(self.transl.shape[0]):
            camera_indiv = Camera(rots[0].cpu().numpy(), transs[0].cpu().numpy(), FovX, FovY, self.img_size, device=self.device)
            self.cameras.append(camera_indiv)

    def load_head_smplx_track(self):
        head_smplx_track_dict = torch.load(os.path.join(self.head_data_dir, 'body_track', 'smplx_track.pth'))
        for key in self.smplx_track_keys:
            self.__setattr__('head_' + key, torch.from_numpy(head_smplx_track_dict[key]).to(self.device))
        self.head_cameras = []
        head_rots = torch.eye(3).float().unsqueeze(0).repeat(self.head_transl.shape[0], 1, 1).to(self.device)
        head_transs = self.head_transl.clone()

        head_cam_para = self.head_cam_para[0]
        head_rots[:, :, 1:] *= -1
        head_transs[:, 1:] *= -1
        fx, fy = float(head_cam_para[0]), float(head_cam_para[1])
        print(head_cam_para, self.head_img_size)
        FovX = focal2fov(fx, self.head_img_size[1])
        FovY = focal2fov(fy, self.head_img_size[0])
        for i in range(self.head_transl.shape[0]):
            head_camera_indiv = Camera(head_rots[i].cpu().numpy(), head_transs[i].cpu().numpy(), FovX, FovY, self.head_img_size, device=self.device)
            self.head_cameras.append(head_camera_indiv)
