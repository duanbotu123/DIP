...
class GaussianScene(nn.Module):
    ...
    def load_smplx_track(self):
        ...
        head_smplx_dict = torch.load(self.head_smplx_path)
        self.jaw_pose = torch.from_numpy(head_smplx_dict['jaw_pose']).to(self.device)[:self.jaw_pose.shape[0]]
        self.leye_pose = torch.from_numpy(head_smplx_dict['leye_pose']).to(self.device)[:self.jaw_pose.shape[0]]
        self.reye_pose = torch.from_numpy(head_smplx_dict['reye_pose']).to(self.device)[:self.jaw_pose.shape[0]]
        self.expr = torch.from_numpy(head_smplx_dict['expr']).to(self.device)[:self.jaw_pose.shape[0]]
