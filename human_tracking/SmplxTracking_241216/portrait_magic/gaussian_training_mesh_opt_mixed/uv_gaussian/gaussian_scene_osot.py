...
class GaussianScene_osot(nn.Module):
    ...
    def cal_lap_loss(self):
        return self.cal_attr_lap_loss(self._features_dc.squeeze(1)) * 1e-3 + self.cal_attr_lap_loss(self._xyz_canonical_dis[0]) + self.cal_attr_lap_loss(self._scaling) + self.cal_attr_lap_loss(self._opacity)
    ...
    def init_attributes(self):
        ...
        self.canonical_scales = gs_neighbor_dist[..., None].repeat(1, 3) * 0.4
        self.canonical_scales[..., 2] *= 0.5
        ...
        opacities = torch.logit(0.5 * torch.ones((pc_num, 1), dtype=torch.float, device=self.device))
