...
from .geometry_osot_ex import GaussianGeo_osot_ex
...
class GaussianScene_osot_iso_ex(nn.Module):
    def __init__(self, img_size, smplx_share, sh_degree = 3, device = 'cuda:0'):
        super().__init__()
        ...
        self.geo_creator = GaussianGeo_osot_ex(device, smplx_share)
    ...
    def update_xyz_rot_osot(self, smplx_inputs, cam_inputs, with_mesh_rendering = True, in_train = False):
        ...
        gs_points_homo = torch.cat((gs_points, torch.ones_like(gs_points[..., :1])), dim=-1)
        self._xyz = torch.bmm(gs_rott.reshape(-1,3,4), gs_points_homo.reshape(-1,4,1)).reshape(batch_size, -1, 3) ### (b, pcs, 3)
        self._xyz += smplx_inputs['trans'].unsqueeze(1)
        # self._xyz = torch.bmm(self._xyz, cam_inputs['R'].permute(0,2,1)) + cam_inputs['t'].unsqueeze(1)
        ...
        verts_homo = torch.cat((verts, torch.ones_like(verts[..., :1])), dim=-1)
        verts_current = torch.bmm(verts_rott.reshape(-1,3,4), verts_homo.reshape(-1,4,1)).reshape(batch_size, -1, 3) ### (b, v_num, 3)
        verts_current += smplx_inputs['trans'].unsqueeze(1)
        # verts_cam = torch.bmm(verts_current, cam_inputs['R'].permute(0,2,1)) + cam_inputs['t'].unsqueeze(1)
        R_cam = cam_inputs['R'].clone()
        R_cam[:,:,1:3]*=-1
        t_cam = cam_inputs['t'].clone()
        t_cam[:,1:3]*=-1
        verts_cam = torch.bmm(verts_current, R_cam.permute(0,2,1)) + t_cam.unsqueeze(1)

        if not with_mesh_rendering:
            return 
        
        # cam_para = cam_inputs['cam_para']
        focal = cam_inputs['focal']
        princpt = cam_inputs['princpt']
