...
class GaussianScene(nn.Module):
    ...
    def cal_body_pose_lap(self):
        body_poses_all = getattr(self, 'body_pose').clone()
        body_poses_all += self._body_pose_dis
        body_poses_all = body_poses_all.reshape(-1, 21, 3)
        joint_lap_weight = body_poses_all.new_ones((1, 21, 1)).detach()
        joint_lap_weight[:, [17, 18, 19, 20, 11, 14], :] = 1e-2
        body_poses_all = body_poses_all*joint_lap_weight
        body_lap = torch.mean((body_poses_all[1:-1] - 0.5*body_poses_all[:-2] - 0.5*body_poses_all[2:])**2)*100
        return body_lap
    ...
    def training_setup(self, training_args):
        ...
        optable_params = [
            {'params': [self._xyz_canonical_dis], 'lr': training_args.position_lr_init * 0., "name": "xyz"},
            {'params': [self._verts_canonical_dis], 'lr': training_args.position_lr_init * 0., "name": "vert_xyz"},
            {'params': [self._features_dc], 'lr': training_args.feature_lr * 0., "name": "f_dc"},
            {'params': [self._features_rest], 'lr': training_args.feature_lr / 20.0 * 0., "name": "f_rest"},
            {'params': [self._opacity], 'lr': training_args.opacity_lr * 0., "name": "opacity"},
            {'params': [self._scaling], 'lr': training_args.scaling_lr * 0., "name": "scaling"},
