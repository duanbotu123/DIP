...
class GaussianTrainer():
    ...
    def eval(self, epoch, filename = None):
        ...
        for img_gt, _, frame_ids, _ in tqdm(self.test_folder_loader, desc='eval'):
    ...
    def train(self):
        epoch_num = max(min(int((25000*40)/len(self.folder_dataset)) + 1, 101), 21)
        train_batch_size = 2
        ...
        decay_rate_epoch = epoch_num // 4 * 3
        save_epoch_freq = 100
        upsh_per_epoch = 10

        loss_keys = ['l1', 'mask', 'mesh_mask', 'ssim', 'G', 'lap', 'dis', 'scale', 'mesh_lap', 'normal_consistent', 'gs_mesh_concistent']
        losses_wts = {'l1': 1.0 - optim_config.lambda_dssim, 'ssim': optim_config.lambda_dssim, 'mask': 1., 'mesh_mask': 1., 'G': .1, 'lap': 1e1, 'dis': 1e-1, 'scale': 1e1, 'mesh_lap': 5e1, 'normal_consistent': 1e-2, 'gs_mesh_concistent': 1e1}
        ...
        for epoch in tqdm(range(epoch_num)):
            ...
            for img_gt, img_parsing, frame_ids, percep_rects in tqdm(folder_loader, total=len(self.folder_dataset)//train_batch_size, desc='training'):
...
#                     '15: eyeg', '16: hat', '17: earr', '18: neck_l'}
                ...
                # head_region = (gt_parsing==1) | (gt_parsing==2) | (gt_parsing==4) | (gt_parsing==5) | (gt_parsing==6) | (gt_parsing==7) | (gt_parsing==8) | (gt_parsing==9) | (gt_parsing==10) | (gt_parsing==11) | (gt_parsing==12) | (gt_parsing==13) | (gt_parsing==14) | (gt_parsing==15) | (gt_parsing==16) | (gt_parsing==17) | (gt_parsing==18)
                mask_for_render_loss = torch.zeros_like(gt_parsing).float()

                # for i in range(mask_for_render_loss.shape[0]):
                #     if frame_ids[i]<=705:
                #         mask_for_render_loss[i][head_region[i]] = 1.0
                #     else:
                #         mask_for_render_loss[i][~head_region[i]] = 1.0

                mask_for_render_loss[...] = 1.
                mask_for_render_loss = mask_for_render_loss.unsqueeze(1)

                #### compute losses
                losses = {}
                losses['l1'] = l1_loss_with_mask(render_img, gt_img, mask_for_render_loss)
                ...
                losses['ssim'] = 1.0 - ssim(render_img*mask_for_render_loss, gt_img*mask_for_render_loss)
                ...
                if epoch > percep_epoch_start:
                    losses['G'] = self.cal_G_loss(render_img*mask_for_render_loss, gt_img*mask_for_render_loss, percep_rects)
                else:
