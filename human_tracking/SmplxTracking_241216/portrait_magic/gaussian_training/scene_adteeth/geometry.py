...
class ADGaussianGeo(nn.Module):
    def __init__(self, device, shape_code, canonical_hair_centers = None):
        ...
        self.skin_part_tids = skin_part_bary_info[:, 0].long()
    ...
    def compute_canonical_points(self):
        ...
        self.canonical_points = torch.cat((canonical_skin_centers, self.canonical_teeth_centers, self.canonical_teeth_centers, self.canonical_hair_centers), dim=0)

    def compute_canonical_points_rott(self, frames_smplx_dict):
        b = frames_smplx_dict['expr'].shape[0]
        smplx_out, verts_rott, verts_canonical = self.smplx_model.forward(betas=self.shape_code.expand(b, -1), body_pose=frames_smplx_dict['body_pose'], left_hand_pose=frames_smplx_dict['lhand_pose'], right_hand_pose=frames_smplx_dict['rhand_pose'], expression=frames_smplx_dict['expr'], jaw_pose=frames_smplx_dict['jaw_pose'], leye_pose=frames_smplx_dict['leye_pose'], reye_pose=frames_smplx_dict['reye_pose'], with_rott_return=True)
        
        canonical_part_verts = verts_canonical[:, self.adnerf_rendering_part_ids, :]
        canonical_skin_centers = bary_inerpolation(canonical_part_verts, self.tris, self.skin_part_tids.unsqueeze(0).expand(b, -1), self.skin_part_coords.unsqueeze(0).expand(b,-1,-1)).squeeze(0)
        self.canonical_points = torch.cat((canonical_skin_centers, self.canonical_teeth_centers, self.canonical_teeth_centers, self.canonical_hair_centers), dim=0)
        ...
        return torch.cat((skin_points_rott, adnerf_part_verts_rott[:, self.upper_teeth_vid].unsqueeze(1).repeat(1, self.teeth_part_pnum, 1, 1), adnerf_part_verts_rott[:, self.downer_teeth_vid].unsqueeze(1).repeat(1, self.teeth_part_pnum, 1, 1), adnerf_part_verts_rott[:, self.hair_vid].unsqueeze(1).repeat(1, self.hair_part_pnum, 1, 1)), dim=1), smplx_out.vertices
