...
class GaussianRenderer(nn.Module):
    ...
    def compute_projection(
        ...
        # Jacobian of perspective projection at each mean (paper eq. 5)
        fx, fy = K[0, 0], K[1, 1]
        tz = cam_points[:, 2].clamp(min=1e-6)  # (N,)
        tx = cam_points[:, 0]                    # (N,)
        ty = cam_points[:, 1]                    # (N,)
        ...
        # covs_cam = W Σ W^T  where W is world-to-camera rotation R
        R_exp = R.unsqueeze(0).expand(N, -1, -1)
        covs_cam = torch.bmm(R_exp, torch.bmm(covs3d, R_exp.transpose(1, 2)))
    ...
    def compute_gaussian_values(
        ...
        # 2x2 analytic inverse & determinant (avoids eigh gradient instability)
        a = covs2D[:, 0, 0]
        b = covs2D[:, 0, 1]
        c = covs2D[:, 1, 1]
        det = (a * c - b * b).clamp(min=1e-10)                   # (N,)
        covs2D_inv = torch.stack([c / det, -b / det,
                                  -b / det,  a / det], dim=-1).reshape(-1, 2, 2)
        normalization = 1.0 / (2 * np.pi * torch.sqrt(det))      # (N,)

        dx_flat = dx.reshape(N, -1, 2)                           # (N, H*W, 2)
        mahal = torch.bmm(dx_flat, covs2D_inv)                    # (N, H*W, 2)
        power = (mahal * dx_flat).sum(dim=-1).reshape(N, H, W)    # (N, H, W)

        gaussian = normalization.view(N, 1, 1) * torch.exp(-0.5 * power.clamp(max=80.0))
    ...
    def forward(
        ...
        ones = torch.ones((1, self.H, self.W), device=alphas.device)
