
# >>> conda initialize >>>
# !! Contents within this block are managed by 'conda init' !!
__conda_setup="$('/nas_data/home/ycz/miniconda3/bin/conda' 'shell.bash' 'hook' 2> /dev/null)"
if [ $? -eq 0 ]; then
    eval "$__conda_setup"
else
    if [ -f "/nas_data/home/ycz/miniconda3/etc/profile.d/conda.sh" ]; then
        . "/nas_data/home/ycz/miniconda3/etc/profile.d/conda.sh"
    else
        export PATH="/nas_data/home/ycz/miniconda3/bin:$PATH"
    fi
fi
unset __conda_setup
# <<< conda initialize <<<

conda activate angs

#不带物体 
# CUDA_VISIBLE_DEVICES=0 python main_avatar.py -c configs/zf/avatar.yaml --mode=test

#带物体
CUDA_VISIBLE_DEVICES=5 python main_avatar.py -c configs/zf/avatar.yaml --mode=hoi